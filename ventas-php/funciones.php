...
function registrarUsuario($usuario, $nombre, $apellidos, $sexo, $fecha_nacimiento, $tipo_documento, $nro_documento, $foto, $direccion, $telefono, $email, $estado, $password){
    $sentencia = "INSERT INTO usuarios (usuario, nombre, apellidos, sexo, fecha_nacimiento, tipo_documento, nro_documento, foto, direccion, telefono, email, estado, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $parametros = [$usuario, $nombre, $apellidos, $sexo, $fecha_nacimiento, $tipo_documento, $nro_documento, $foto['name'], $direccion, $telefono, $email, $estado, password_hash($password, PASSWORD_DEFAULT)];
...
function editarCliente($nombre, $apellidos, $tipo_documento, $nro_documento, $telefono, $direccion, $email, $id){
    $sentencia = "UPDATE clientes SET nombre = ?, apellidos = ?, tipo_documento = ?, nro_documento = ?, telefono = ?, direccion = ?, email = ? WHERE id = ?";
    $parametros = [$nombre, $apellidos, $tipo_documento, $nro_documento, $telefono, $direccion, $email, $id];
    return editar($sentencia, $parametros);
}
...
//ESTO MODIFIQUÉ 
function registrarCliente($nombre, $apellidos, $tipo_documento, $nro_documento, $telefono, $direccion, $email){
    $sentencia = "INSERT INTO clientes (nombre, apellidos, tipo_documento, nro_documento, telefono, direccion, email) VALUES (?,?,?,?,?,?,?)";
    $parametros = [$nombre, $apellidos, $tipo_documento, $nro_documento, $telefono, $direccion, $email];
    return insertar($sentencia, $parametros);
}
...
function registrarProducto($codigo, $nombre, $compra, $venta, $existencia, $descripcion, $marca, $modelo, $stock_inicial, $stock_actual, $categoria, $unidad_medida){
    $sentencia = "INSERT INTO productos(codigo, nombre, compra, venta, existencia, descripcion, marca, modelo, stock_inicial, stock_actual, categoria, unidad_medida) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
    $parametros = [$codigo, $nombre, $compra, $venta, $existencia, $descripcion, $marca, $modelo, $stock_inicial, $stock_actual, $categoria, $unidad_medida];
    return insertar($sentencia, $parametros);
}
...
//FUNCIONES CREADAS PARA LOS PROVEEDORES
...
function registrarProveedor($razon_social, $ruc, $direccion, $telefono, $email, $url){
    $sentencia = "INSERT INTO proveedores (razon_social, ruc, direccion, telefono, email, url) VALUES (?,?,?,?,?,?)";
    $parametros = [$razon_social, $ruc, $direccion, $telefono, $email, $url];
    return insertar($sentencia, $parametros);
}

function editarProveedor($razon_social, $ruc, $direccion, $telefono, $email, $url, $id){
    $sentencia = "UPDATE proveedores SET razon_social = ?, ruc = ?, direccion = ?, telefono = ?, email = ?, url = ? WHERE id = ?";
    $parametros = [$razon_social, $ruc, $direccion, $telefono, $email, $url, $id];
    return editar($sentencia, $parametros);
}
...
//FUNCIONES CRADAS PARA LOS PROVEEDORES 


//RESPECTO A COMPRAS 
function registrarCompra($tipo_comprobante, $nro_comprobante, $fecha_emision, $proveedor_id, $detalles){
    $pdo = conectarBaseDatos();
    try {
        $pdo->beginTransaction();

        $sentencia = "INSERT INTO compras (tipo_comprobante, nro_comprobante, fecha_emision, proveedor_id) VALUES (?,?,?,?)";
        $stmt = $pdo->prepare($sentencia);
        $stmt->execute([$tipo_comprobante, $nro_comprobante, $fecha_emision, $proveedor_id]);
        $compra_id = $pdo->lastInsertId();

        foreach ($detalles as $detalle) {
            $sentencia = "INSERT INTO detalle_compras (compra_id, producto_id, cantidad, precio) VALUES (?,?,?,?)";
            $stmt = $pdo->prepare($sentencia);
            $stmt->execute([$compra_id, $detalle['producto_id'], $detalle['cantidad'], $detalle['precio']]);

            $sentencia = "UPDATE productos SET stock_actual = stock_actual + ? WHERE id = ?";
            $stmt = $pdo->prepare($sentencia);
...
function anularCompra($compra_id){
    $pdo = conectarBaseDatos();
    try {
        $pdo->beginTransaction();

        $sentencia = "SELECT producto_id, cantidad FROM detalle_compras WHERE compra_id = ?";
        $stmt = $pdo->prepare($sentencia);
        ...
        $sentencia = "DELETE FROM detalle_compras WHERE compra_id = ?";
        $stmt = $pdo->prepare($sentencia);
        $stmt->execute([$compra_id]);

        $sentencia = "DELETE FROM compras WHERE id = ?";
        $stmt = $pdo->prepare($sentencia);
        $stmt->execute([$compra_id]);
...
function obtenerCompras(){
    $sentencia = "SELECT c.*, p.razon_social as proveedor FROM compras c JOIN proveedores p ON c.proveedor_id = p.id";
    $compras = select($sentencia);

    foreach ($compras as $compra) {
        $sentencia = "SELECT dc.*, pr.nombre as producto FROM detalle_compras dc JOIN productos pr ON dc.producto_id = pr.id WHERE dc.compra_id = ?";
        $detalles = select($sentencia, [$compra->id]);
...
//RESPECTO A GENERAR REPORTE COMPRAS

function obtenerComprasEntreFechas($fecha_inicio, $fecha_fin){
    $sentencia = "SELECT c.*, p.razon_social as proveedor FROM compras c 
                  JOIN proveedores p ON c.proveedor_id = p.id 
                  WHERE c.fecha_emision BETWEEN ? AND ?";
    $compras = select($sentencia, [$fecha_inicio, $fecha_fin]);

    foreach ($compras as $compra) {
        $sentencia = "SELECT dc.*, pr.nombre as producto FROM detalle_compras dc 
